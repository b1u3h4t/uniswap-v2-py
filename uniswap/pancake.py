...
class PancakeUtils(object):
    ...
    def sort_tokens(token_a, token_b):
        ...
        assert token_0 != PancakeUtils.ZERO_ADDRESS
        return token_0, token_1

    @staticmethod
    def pair_for(factory, token_a, token_b):
        prefix = Web3.toHex(hexstr="ff")
        encoded_tokens = Web3.solidityKeccak(["address", "address"], PancakeUtils.sort_tokens(token_a, token_b))
        # init code pair hash
        suffix = Web3.toHex(hexstr="d0d4c4cd0848c93cb4fd1f498d7013ee6bfb25783ea21593d5834f5d250ece66")
    ...
    def get_reserves(factory, token_a, token_b):
        pass  # TODO move to PancakeClient
...
class PancakeClient(UniswapObject):
    ...
    def __init__(self, address, private_key, provider=None):
        ...
        self.contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(PancakeClient.ADDRESS), abi=PancakeClient.ABI)
        self.router = self.conn.eth.contract(
            address=Web3.toChecksumAddress(PancakeClient.ROUTER_ADDRESS), abi=PancakeClient.ROUTER_ABI)

    # Utilities
    # -----------------------------------------------------------
    def _is_approved(self, token, amount=MAX_APPROVAL_INT):
        erc20_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(token), abi=PancakeClient.PAIR_ABI)
    ...
    def approve(self, token, max_approval=MAX_APPROVAL_INT):
        ...
        erc20_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(token), abi=PancakeClient.ERC20_ABI)
    ...
    def remove_liquidity_eth(self, token, liquidity, min_token, min_eth, to, deadline):
        ...
        """
        self.approve(self.get_pair(token, "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c"), liquidity)  # FIXME hardcoded WETH address
    ...
    def get_token_0(self, pair):
        ...
        pair_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(pair), abi=PancakeClient.PAIR_ABI)
